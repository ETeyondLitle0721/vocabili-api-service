import { repair_character as repair } from "./base.js";
...
function _replacer(table, match, index) {
    const parameter = match[1].split(/\s*,\s*/);
    ...
    const length = parseInt(parameter[1]) || 1,
...
function _parser(before, after, match, index) {
    const suffix = after.slice(match.index + match[0].length),
    ...
    let value = before.match(regexp)[1] ?? null;
...
export function replace(expression, config, replacer = _replacer) {
    const match = expression.matchAll(regex.match);
...
export function parse(before, after, parser = _parser) {
    const result = {}, match = after.matchAll(regex.match);

    if (match) {
        const list = Array.from(match);

        for (let index = 0; index < list.length; index++) {
            const { field, ...parameter } = parser(
