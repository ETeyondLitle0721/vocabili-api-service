...
export function get_point(rate, offset) {
    ...
    return Object.assign(
        point, {
            "_": floor(point.view + point.coin + point.like + point.favorite)
        }
    );
}
...
calculator.register("calculator-0001", issue => issue <= 20240711, offset => {
    ...
    const rate = {
        "view": view <= 0 ? 0 : ceil(((coin + favorite + like) / view) * 100) / 100,
        "coin": coin <= 0 ? 0 : min(40, ceil((coin * 100 + view) / (coin * 100) * 10 * 100) / 100),
        "like": like * 20 + view <= 0 ? 0 : floor(((coin + favorite) * 100) / (like * 20 + view) * 100) / 100,
        "favorite": favorite * 20 + view <= 0 ? 0 : min(20, ceil(favorite * 20 / (favorite * 20 + view) * 100) / 100)
    };
...
calculator.register("calculator-0002", issue => issue >= 20240712 && issue <= 20241101, (offset, copyright) => {
    ...
    const rate = {
        "view": view <= 0 ? 0 : max(1, ceil(((coin + favorite * 20) / view * 100) / 100)),
        "coin": state * coin * 40 + view <= 0 ? 0 : max(40, ceil(state * coin * 40 / (state * coin * 40 + view) * 80 * 100) / 100),
        "like": like * 20 + view <= 0 ? 0 : floor((coin + favorite) / (like * 20 + view) * 100 * 100) / 100,
        "favorite": favorite * 20 + view <= 0 ? 0 : max(20, ceil((favorite + 2 * coin) / (favorite * 20 + view) * 100) / 100)
    };
...
calculator.register("calculator-0003", issue => issue > 20241101, (offset, copyright) => {
    ...
    const fix = {
        "a": coin <= 0 ? 0 : (copyright === 1 ? 1 : ceil(max(1, (view + 20 * favorite + 40 * coin + 10 * like) / (200 * coin)) * 100) / 100),
        "b": (view + 20 * favorite) <= 0 ? 0 : ceil(min(1, 3 * max(0, (20 * coin + 10 * like)) / (view + 20 * favorite)) * 100) / 100,
    };

    fix.c = like + favorite <= 0 ? 0 : ceil(min(1, (like + favorite + 20 * coin * fix.a) / (2 * like + 2 * favorite)) * 100) / 100;
    fix._ = round(fix.b * fix.c * 100) / 100;

    const rate = {
        "view": view <= 0 ? 0 : max(ceil(min(max((fix.a * coin + favorite), 0) * 20 / view, 1) * 100) / 100, 0),
        "coin": fix.a * coin * 40 + view <= 0 ? 0 : max(ceil(min((fix.a * coin * 40) / (fix.a * coin * 40 + view) * 80, 40) * 100) / 100, 0),
        "like": like <= 0 ? 0 : max(floor(min(5, max(fix.a * coin + favorite, 0) / (like * 20 + view) * 100) * 100) / 100, 0),
        "favorite": favorite <= 0 ? 0 : max(ceil(min((favorite + 2 * fix.a * coin) * 10 / (favorite * 20 + view) * 40, 20) * 100) / 100, 0)
    };
    ...
    point._ = floor(round(point.view + point.like + point.coin + point.favorite) * fix.b * fix.c);
